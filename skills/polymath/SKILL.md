...
<!-- inline shell (was: !`cat .production-grade.yaml 2>/dev/null || echo "No config"`) -->
Run shell command before continuing: ``cat .production-grade.yaml 2>/dev/null || echo "No config"``
(use the `execute_shell_command` tool).
<!-- inline shell (was: !`cat Claude-Production-Grade-Suite/polymath/context/decisions.md 2>/dev/null || echo "No prior polymath context"`) -->
Run shell command before continuing: ``cat Claude-Production-Grade-Suite/polymath/context/decisions.md 2>/dev/null || echo "No prior polymath context"``
(use the `execute_shell_command` tool).
<!-- inline shell (was: !`cat Claude-Production-Grade-Suite/polymath/context/repo-map.md 2>/dev/null || echo "No repo map"`) -->
Run shell command before continuing: ``cat Claude-Production-Grade-Suite/polymath/context/repo-map.md 2>/dev/null || echo "No repo map"``
(use the `execute_shell_command` tool).
...
```
tavily_search("[domain] platforms 2026 comparison")
tavily_search("[domain] market size growth trends")
tavily_search("[domain] pain points challenges")
tavily_search("[domain] technology stack patterns")
```

**Deep Dive** — follow up on specific findings:
```
tavily_search("[specific topic]")
→ finds relevant page
tavily_search("[url]")
...
```
tavily_search("[specific claim] accuracy [year]")
→ cross-reference 2-3 sources
```

**Cost Modeling** — real numbers, not guesses:
```
tavily_search("[cloud service] pricing [year]")
tavily_search("[competitor] pricing plans")
```
