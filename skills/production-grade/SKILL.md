...
<!-- inline shell (was: !`git status 2>/dev/null || echo "No git repo detected"`) -->
Run shell command before continuing: ``git status 2>/dev/null || echo "No git repo detected"``
(use the `execute_shell_command` tool).
<!-- inline shell (was: !`cat CLAUDE.md 2>/dev/null || echo "No CLAUDE.md found"`) -->
Run shell command before continuing: ``cat CLAUDE.md 2>/dev/null || echo "No CLAUDE.md found"``
(use the `execute_shell_command` tool).
<!-- inline shell (was: !`ls Claude-Production-Grade-Suite/ 2>/dev/null || echo "No existing workspace"`) -->
Run shell command before continuing: ``ls Claude-Production-Grade-Suite/ 2>/dev/null || echo "No existing workspace"``
(use the `execute_shell_command` tool).
<!-- inline shell (was: !`cat .production-grade.yaml 2>/dev/null || echo "No config file — defaults apply"`) -->
Run shell command before continuing: ``cat .production-grade.yaml 2>/dev/null || echo "No config file — defaults apply"``
(use the `execute_shell_command` tool).
...
```python
<!-- v0.1: render numbered options as plain Markdown; parse reply --> AskUserQuestion(questions=[{
...
- **Cleanup:** After mode completion (or gate rejection), run `<!-- v0.1: no-op --> TeamDelete(team_name="production-grade")` if a team was created. Never leave orphaned agents.
...
4. **Codebase discovery — detect greenfield vs brownfield:**
   ...
   ```python
   glob_search("package.json"), glob_search("go.mod"), glob_search("pyproject.toml"), glob_search("Cargo.toml"), glob_search("pom.xml")
   glob_search("src/**"), glob_search("services/**"), glob_search("frontend/**"), glob_search("tests/**"), glob_search("docs/**")
   glob_search("Dockerfile*"), glob_search(".github/workflows/*"), glob_search("infrastructure/**"), glob_search("terraform/**")
   glob_search(".production-grade.yaml")
   ```
...
8. **Polymath pre-flight check:**
   ...
   - If no polymath context, assess the user's request for knowledge gaps:
     ...
     - If gaps detected → invoke `<!-- v0.1: read the sub-skill SKILL.md inline --> Skill("polymath")` for pre-flight consultation before proceeding. The polymath will research, clarify with the user, and write a context package when ready.
...
```python
<!-- v0.1: no-op --> TeamCreate(team_name="production-grade")
```
...
```python
<!-- v0.1: read the sub-skill SKILL.md inline --> Skill(skill="polymath")
...
```python
<!-- v0.1: read the sub-skill SKILL.md inline --> Skill(skill="product-manager")
```
...
**How:** Use `glob_search` to list files, `read_file` to load content. If a file doesn't exist, skip it — don't error. Then create agent task prompts using the freshly-read data, not compressed memory.
...
```python
<!-- v0.1: no-op --> TeamDelete(team_name="production-grade")
```
...
**If the user rejects at any gate** (Gate 1, 2, or 3), also run `(no-op in v0.1)` before stopping. Never leave orphaned agents.
...
| Not calling TeamDelete after completion | ALWAYS run `<!-- v0.1: no-op --> TeamDelete(team_name="production-grade")` after final summary or gate rejection. Orphaned agents idle forever. |
