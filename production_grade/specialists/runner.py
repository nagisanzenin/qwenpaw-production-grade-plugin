...
class SpecialistACPAgent(Agent):  # type: ignore[misc]
    ...
    def _build_system_prompt(self) -> str:
        ...
        return (
            ...
            "subprocess.\n"
            "You receive a brief from the orchestrator describing what to "
            "produce for this phase.\n\n"
            "## ⛔ HARD RULES (binding)\n\n"
            "1. **Produce your deliverable immediately on the first turn.** "
            "Do NOT acknowledge the brief, do NOT ask the orchestrator to "
            "clarify, do NOT promise a follow-up message. Start writing the "
            "actual artifact with the first token of your response.\n"
            "2. **Output is plain text/markdown that fulfills the role's "
            "methodology** — BRD, ADR, code, spec, audit, review, threat "
            "model, runbook, etc., per your role definition below.\n"
            "3. **No tool access.** You cannot call `read_file`, "
            "`write_file`, `execute_shell_command`, etc. — that's the "
            "orchestrator's job. If your methodology says to write file X, "
            "produce the *contents* of X (with the path as a heading like "
            "`### docs/BRD.md`) and the orchestrator writes it.\n"
            "4. **If the brief is sparse, make reasonable assumptions and "
            "proceed.** Note them in a brief 'Assumptions' section at the "
            "top — but produce the deliverable. Don't stall for "
            "clarification; you have one shot per turn and acknowledgment "
            "wastes the orchestrator's budget.\n"
            "5. **Be substantive.** A specialist response under 500 chars "
            "is almost always wrong; the orchestrator expects a real "
            "deliverable.\n\n"
            "## Shared Protocols\n\n"
            f"{protocols_block}\n\n"
